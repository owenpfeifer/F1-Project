...
st.title("⚔️ Compare Driver Telemetry in Qualifying")
...
with col2:
    ...
    selected_race = st.selectbox("Select Weekend", races)
    ...
    if driver_1 == driver_2:
        ...
        with st.spinner(f"Fetching telemetry for {driver_1} and {driver_2}..."):
            ...
            try:
                ...
                ax.set_title(f"Fastest Qualifying Lap Speed Comparison - {selected_year} {selected_race}")
                ax.legend()
                
                st.pyplot(fig)
                
            except Exception as e:
                st.error(f"Could not load telemetry. Ensure both drivers completed a lap in this qualifying session. Error: {e}")